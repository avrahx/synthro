...
const TABS: { id: Tab; label: string; icon: React.ElementType; badge?: boolean }[] = [
  { id: "live",      label: "LIVE MARKET",        icon: Sparkles },
  { id: "portfolio", label: "PORTFOLIO AUDIT",     icon: Wallet },
  { id: "ml_regime", label: "ML REGIME ENGINE",    icon: BrainCircuit },
  { id: "backtest",  label: "BACKTEST ENGINE",     icon: BarChart3 },
  { id: "vault",     label: "VAULT SIMULATOR",     icon: Vault },
  { id: "execution", label: "EXECUTION TERMINAL",  icon: Terminal },
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link
                href="/"
                className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 hover:border-white/25 text-gray-400 hover:text-white font-mono text-xs transition-all"
              >
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg bg-bg-raised border border-border-subtle w-fit font-mono text-xs overflow-x-auto max-w-full">
          {TABS.map((t) => (
            ...
            >
              ...
              <span>{t.label}</span>
              {(t.id === "portfolio") && (isConnected || isDemoMode) && tab !== t.id && (
          ...
          <div className="flex items-center gap-2 text-gray-300">
            ...
            <span className="text-white font-bold">
              {isConnected && address
                ...
                : "Whale Portfolio (0xdf84...4261)"}
            </span>
            <span className="hidden lg:inline text-gray-500">
              — Real-time delta exposure, 1H funding carry, and liquidation health available.
            </span>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass rounded-xl p-6 border border-border-subtle h-[500px]">
              ...
              <p className="text-gray-400 font-mono text-sm leading-relaxed mb-4">
                The Synthro HyperVault utilizes a customized ERC-4626 implementation
                that anchors all state proofs on-chain.
              </p>
              ...
              <div className="glass rounded-xl p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 font-mono text-xs">
                  {result.attribution.map((a) => (
                    <div
                      key={a.asset}
                      className="p-4 rounded-lg bg-bg-elevated border border-border-subtle space-y-2 hover:border-hl-cyan/30 transition-all"
                    >
                      <div className="flex items-center justify-between border-b border-border-subtle pb-2">
                        <span className="font-bold text-white text-sm">{a.asset}</span>
                        <span className="px-2 py-0.5 rounded bg-bg text-[10px] text-gray-500">
                          {a.trade_count} trades
                        </span>
                      </div>
                      <div className="flex justify-between text-gray-400">
                        <span>Funding:</span>
                        <span className="text-hl-green font-semibold">
                          +${a.funding_earned.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-gray-400">
                        <span>Basis PnL:</span>
